 ...
 *  student 클래스를 선언하고 new Student()를 호출하면 새로운 객체가 생성되며 넘겨받는 인수 name과 함께constructor가 자동으로 실행되어 "홍길동"이 this.name에 할당한다.
 * 클래스 몸체에 정의한 메서드는 기본적으로 프로토타입 메서드가 된다. */

class Student {
  
  // 생성자를 통해 인스턴스 생성 및 초기화
  // 생성자는 1개 이상 정의 될 수 없으며 생략할 경우 암묵적으로 정의 된다. 암묵적으로 this를 반환하도록 작성하지 않는다.
    constructor(name){
...
console.log(Student.prototype.introduce);   // [Function: introduce] - 클래스 내부에서 정의한 메서드는 클래스.prototype에 저장된다.
console.log(Object.getOwnPropertyNames(Student.prototype));  // [ 'constructor', 'introduce' ] - 현재 프로토타입에는 constructor, introduce 두개의 매서드가 존재
...
/** 생성자 함수와 클래스의 차이점
 * 1. 클래스 생성자를 new와 함께 호출하지 않으면 에러가 발생한다. 함수 내부 프로퍼티 [[IsClassConstructor]]:true 가 사용된다.
 * 2. 클래스에 정의 된 메서드는 열거가 불가하다. enumerable 플래그가 false이기 때문이다. 따라서 for..in 으로 객체 순회 시 메서드 순회 대상에서 제외된다.
 * 3. 클래스는 호이스팅이 발생하지 않는 것처럼 동작한다. 함수 선언문 생성자 함수는 호이스팅, 함수 표현식 생성자 함수는 변수 호이스팅이 발생한다.
 * 4. 클래스는 항상 use strict 적용된다. 클래스 생성자 안 코드는 자동으로 엄격 모드가 적용 된다.
 * 
 * 따라서 클래스 문법은 생성자 함수와는 다른 새로운 객체 생성 메커니즘이다. */
...
let computer = new Product('컴퓨터', -1600000);
...
console.log(Book.prototype.name);   // Book.prototype이 아닌 객체에만 클래스 필드가 설정됨
